...
fn fisher_yates_shuffle(v: &mut Vec<(f32, crate::individual::Individual)>, rng: &mut StdRand) {
    let v_len: u16 = v.len() as u16;
    for i in 0..v_len {
        v.swap(i as usize, rng.next_lim_u16(v_len) as usize);
    }
}
...
fn main() {
    ...
    for generation in 0..max_gen {
        println!(
            ...
            SystemTime::now()
                ...
                .as_secs(),
            generation,
            population.iter().max_by(|ind0, ind1| ind0.0.total_cmp(&ind1.0)).unwrap().0,
        );
        ...
        fisher_yates_shuffle(&mut population, &mut rng);

    }
    println!("Writing best individual to ./result.wav");
    let mut outfile =
        File::create(Path::new("./result.wav")).expect("Cannot open file for writing");
    ...
    wav::write(
        header,
        &wav::bit_depth::BitDepth::from(population[0].1.to_wave().samples.to_vec()),
        &mut outfile,
    )
    .expect("Cannot write wave");
}
