...
use tinyrand::{StdRand, RandRange, Rand, Probability};
use vosk::Recognizer;
...
impl Individual {
    pub fn new_rand(rng: &mut StdRand) -> Self {
        ...
        for _ in 0..rng.next_range(crate::consts::MIN_WAVES..crate::consts::MAX_WAVES) {
            ...
            ind.waves.insert(crate::sinewave::SineWave::new(start, length, freq));
        }
        ind
    }
    ...
    pub fn fitness(&self, recognizer: &mut Recognizer) -> f32 {
        ...
        let ft: u16 = match recognizer.final_result().single() {
            Some(result) => lv::distance(crate::consts::TARGET_WORD.as_bytes(), result.text.as_bytes()).0 as u16,
    ...
    pub fn combine(&self, other: &Individual, rng: &mut StdRand) -> (Individual, Individual) {
        ...
        let mut ind0 = Individual { waves: HashSet::new() };
        let mut ind1 = Individual { waves: HashSet::new() };
